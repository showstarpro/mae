...
OMP_NUM_THREADS=1 torchrun --nproc-per-node=8 --master-port=29501 main_pretrain.py \
    ...
    --shrink_num 147 \
    --ncrop_loss 3 \
    --output_dir /lpai/output/models/lyy/pami_sdae_200ep/pre \
    --log_dir /lpai/output/models/lyy/pami_sdae_200ep/pre ;

OMP_NUM_THREADS=1 torchrun --nproc-per-node=8 --master-port=29502 main_finetune.py \
    --accum_iter 1 \
    --batch_size 128 \
    --model vit_base_patch16 \
    --finetune /lpai/output/models/lyy/pami_sdae_200ep/pre/checkpoint-199.pth \
    --epochs 100 \
    --warmup_epochs 5 \
    --blr 1e-3 \
    --layer_decay 0.75 \
    ...
    --dist_eval \
    --data_path $LPAI_INPUT_DATASET_0 \
    --output_dir /lpai/output/models/lyy/pami_sdae_200ep/fin \
    --log_dir /lpai/output/models/lyy/pami_sdae_200ep/fin ;
...
OMP_NUM_THREADS=1 torchrun --nproc-per-node=8 --master-port=29503 main_linprobe.py \
    ...
    --dist_eval \
    --data_path $LPAI_INPUT_DATASET_0 \
    --output_dir /lpai/output/models/lyy/pami_sdae_200ep/ln \
    --log_dir /lpai/output/models/lyy/pami_sdae_200ep/ln ;
