...
def convert_beit(ckpt):
    ...
    for k, v in ckpt.items():
        if k.startswith('patch_embed'):
            new_key = k.replace('patch_embed.proj', 'backbone.patch_embed.projection')
            new_ckpt[new_key] = v
        elif k.startswith('cls_token'):
            new_key = k.replace('cls_token', 'backbone.cls_token')
            new_ckpt[new_key] = v
        elif k.startswith('pos_embed'):
            new_key = k.replace('pos_embed', 'backbone.pos_embed')
            new_ckpt[new_key] = v
        elif k.startswith('blocks'):
            new_key = k.replace('blocks', 'backbone.layers')
            if 'norm' in new_key:
